...
export function registerHeartRateTools(
    ...
    server.tool(
        ...
        async ({ period, date = "today" }: HeartRatePeriodParams): Promise<ToolResponseStructure> => {

            // Construct the endpoint dynamically
            const endpoint = `activities/heart/date/${date}/${period}.json`;

            const heartRateData = await makeFitbitRequest<HeartRateTimeSeriesResponse>(
                endpoint, 
                getAccessTokenFn,
                FITBIT_API_BASE
            );
            ...
            if (!heartRateData) {
                return {
                    content: [{ type: "text", text: `Failed to retrieve heart rate data from Fitbit API for date '${date}' and period '${period}'. Check token and permissions.` }],
                    isError: true
                };
            }
            ...
            if (heartRateEntries.length === 0) {
                return {
                    content: [{ type: "text", text: `No heart rate data found for date '${date}' and period '${period}'.` }]
                };
            }
    ...
    server.tool(
        ...
        async ({ startDate, endDate }: HeartRateRangeParams): Promise<ToolResponseStructure> => {
            ...
            if (!heartRateData) {
                return {
                    content: [{ type: "text", text: `Failed to retrieve heart rate data from Fitbit API for the date range '${startDate}' to '${endDate}'. Check token, permissions, date format, and ensure the range is 1 year or less.` }],
                    isError: true
                };
            }
            ...
            if (heartRateEntries.length === 0) {
                return {
                    content: [{ type: "text", text: `No heart rate data found for the date range '${startDate}' to '${endDate}'.` }]
                };
            }
