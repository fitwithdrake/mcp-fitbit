...
export function registerNutritionTools(
    ...
    server.tool(
        ...
        async ({ date = "today" }: FoodLogParams): Promise<ToolResponseStructure> => {
            ...
            if (!foodLogData) {
                return {
                    content: [{ type: "text", text: `Failed to retrieve food log data from Fitbit API for date '${date}'. Check token and permissions.` }],
                    isError: true
                };
            }
    ...
    const periodParametersSchemaShape = {
        resource: z.enum(["caloriesIn", "water", "protein", "carbs", "fat", "fiber", "sodium"])
                   .describe("The nutrition resource to retrieve data for."),
        period: z.enum(["1d", "7d", "30d", "1w", "1m", "3m", "6m", "1y"])
    ...
    type NutritionPeriodParams = {
        resource: "caloriesIn" | "water" | "protein" | "carbs" | "fat" | "fiber" | "sodium";
    ...
    server.tool(
        ...
        async ({ resource, period, date = "today" }: NutritionPeriodParams): Promise<ToolResponseStructure> => {

            // Construct the endpoint dynamically
            const endpoint = `foods/log/${resource}/date/${date}/${period}.json`;

            const nutritionData = await makeFitbitRequest<NutritionTimeSeriesResponse>(
                endpoint, 
                getAccessTokenFn,
                FITBIT_API_BASE
            );
            ...
            if (!nutritionData) {
                return {
                    content: [{ type: "text", text: `Failed to retrieve nutrition data from Fitbit API for resource '${resource}', date '${date}' and period '${period}'. Check token and permissions.` }],
                    isError: true
                };
            }
            ...
            if (nutritionEntries.length === 0) {
                return {
                    content: [{ type: "text", text: `No nutrition data found for resource '${resource}', date '${date}' and period '${period}'.` }]
                };
            }
    ...
    server.tool(
        ...
        async ({ resource, startDate, endDate }: NutritionRangeParams): Promise<ToolResponseStructure> => {
            ...
            if (!nutritionData) {
                return {
                    content: [{ type: "text", text: `Failed to retrieve nutrition data from Fitbit API for resource '${resource}' and the date range '${startDate}' to '${endDate}'. Check token, permissions, date format, and ensure the range is 1,095 days or less.` }],
                    isError: true
                };
            }
            ...
            if (nutritionEntries.length === 0) {
                return {
                    content: [{ type: "text", text: `No nutrition data found for resource '${resource}' and the date range '${startDate}' to '${endDate}'.` }]
                };
            }
